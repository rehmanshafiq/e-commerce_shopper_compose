package com.shafiq.shopper_ecommerce.ui.feature.user_address
...
fun UserAddressScreen(navController: NavController, userAddress: UserAddress?) {
    ...
    Column(modifier = Modifier
        ...
        Button(onClick = {
            ...
            previousBackStack?.savedStateHandle?.set(USER_ADDRESS, address)
            navController.popBackStack()
        },
            modifier = Modifier.fillMaxWidth(),
            enabled = addressLine.value.isNotEmpty() && city.value.isNotEmpty()
                    && state.value.isNotEmpty() && postalCode.value.isNotEmpty() &&
